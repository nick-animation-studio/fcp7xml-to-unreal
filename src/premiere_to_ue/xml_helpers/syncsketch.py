...
def connect_to_syncsketch():
    ...
    if not SYNCSKETCH_USERNAME or not SYNCSKETCH_API_KEY:
        logger.error("syncsketch .env file not configured properly, missing values.")
        return
    else:
        logger.info(f"INFO: using SYNCSKETCH_USERNAME: {SYNCSKETCH_USERNAME}")
        logger.info(f"INFO: using SYNCSKETCH_API_KEY: {SYNCSKETCH_API_KEY}")
...
def get_project_id(s):
    ...
    if len(projects) != 1:
        logger.error("there is not exactly one project associated with this account")
        return None
...
def get_review_id(s, project_id, syncsketch_link):
    ...
    for review in reviews:
        if review["reviewURL"] in syncsketch_link:
            return review["id"]
    logger.error("can't find a review matching the URL provided")
    return None
...
def get_item_id(s, review_id, syncsketch_link):
    ...
    for item in items:
        if str(item["id"]) in syncsketch_link:
            return item["id"]
    logger.error("can't find an item matching the URL provided")
    return None
...
def get_name(syncsketch_link):
    ...
    if s:
        logger.info("Successfully connected to syncsketch")
    else:
        logger.error("could not connect to syncsketch, check console for errors.")
        return
...
def upload(episode: Episode, syncsketch_link: str):
    ...
    logger.info(f"Found {len(episode.notes)} comments to upload")
    ...
    logger.info("Successfully uploaded notes to syncsketch")
